<div class="heading-section heading-section-1 dark">
    <h3>Latest News</h3>
</div>
<div class="owl-carousel dark nav-style-1" data-carousel-margin="45" data-carousel-nav="true" data-carousel-loop="true">
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">
            </a>
        </div>
        <div class="date">
            <span>July 22, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Improve margins and reposition quickly</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">

            </a>
        </div>
        <div class="date">
            <span>July 18, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Delight customers and operate faster</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">
            </a>
        </div>
        <div class="date">
            <span>July 12, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Strategy experience and analytical</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">
            </a>
        </div>
        <div class="date">
            <span>July 22, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Improve margins and reposition quickly</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">

            </a>
        </div>
        <div class="date">
            <span>July 18, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Delight customers and operate faster</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
    <div class="image-card image-card-4">
        <div class="image">
            <a href="{{ url('/single-blog') }}">
                <img src="https://picsum.photos/1600/870?random={{ rand(1, 100) }}" alt="image">
            </a>
        </div>
        <div class="date">
            <span>July 12, 2018</span>
        </div>
        <h3 class="title">
            <a href="{{ url('/single-blog') }}">Strategy experience and analytical</a>
        </h3>
        <div class="content">
            <p>Phasellus lorem enim, luctus ut velit eget, convallis egestas eros. Sed ornare ligula
                eget tortor tempor, quis porta tellus dictum.</p>
        </div>
        <div class="link">
            <a href="{{ url('/single-blog') }}">Continue reading</a>
        </div>
    </div>
</div>
